...
} from '../src/markdownLanguageServerRequest.ts'
...
const documents = new Map([[readmeUri, { text: '# In-memory heading' }]])
const context = {
  getDocument: (uri: string): { readonly text: string } | undefined => documents.get(uri),
  rootUri,
}
...
test('handles Markdown file watcher requests', async () => {
...
test('parses the current in-memory Markdown document', async () => {
  const tokens = (await executeMarkdownLanguageServerRequest(
    'markdown/parse',
    { uri: readmeUri },
    context,
  )) as readonly {
...
test('reads workspace files as bytes', async () => {
  const result = (await executeMarkdownLanguageServerRequest(
...
test('reads workspace directories', async () => {
  const result = (await executeMarkdownLanguageServerRequest(
    ...
    { uri: rootUri },
    context,
  )) as readonly [string, { readonly isDirectory: boolean }][]
...
test('finds Markdown files in the workspace', async () => {
  await expect(
    executeMarkdownLanguageServerRequest('markdown/findMarkdownFilesInWorkspace', {}, context),
  ).resolves.toEqual([
...
test('rejects file requests outside the workspace', async () => {
  await expect(
    executeMarkdownLanguageServerRequest(
      'markdown/fs/readFile',
      { uri: pathToFileURL('/tmp/outside.md').href },
      context,
    ),
