// cspell:ignore mdtext mdtxt mdwn mkdn
...
export interface MarkdownLanguageServerRequestContext {
  readonly getDocument: (uri: string) => Readonly<DocumentState> | undefined
...
const markdownLanguageServerRequests = [
  'markdown/parse',
...
export const isMarkdownLanguageServerRequest = (method: string): boolean => {
  return markdownLanguageServerRequests.includes(method as (typeof markdownLanguageServerRequests)[number])
}
...
const getMarkdownText = async (
  params: Readonly<MarkdownRequestParams>,
  context: Readonly<MarkdownLanguageServerRequestContext>,
): Promise<string> => {
  ...
  const document = context.getDocument(params.uri)
  if (document) {
